...
#' Create a free CDS user account by
#' \href{https://cds.climate.copernicus.eu/user/register}{self registering}.
...
#'
#' Requires the suggested package \pkg{ecmwfr} (and \pkg{sf} when `shape` is
#' supplied).
#'
...
#' @param user character; the user ID used to store the CDS key with
#' [ecmwfr::wf_set_key()].
#' @param era5_dataset string; which ERA5 dataset to use. Can be
#' 'reanalysis-era5-single-levels' or 'reanalysis-era5-land'.
#' @param path filepath to store downloaded file.
#'
#' @return A data frame of the submitted requests, as returned by
#' [ecmwfr::wf_request_batch()].
#'
#' @seealso [read_era5_grib_point()] to extract a point time series from the
#' downloaded files.
#'
#' @examples
#' \dontrun{
...
#' }
#' @export
download_era5_cds <- function(shape = NULL,
                              lat,
                              lon,
                              buffer = 0.1,
  ...
  if (!requireNamespace("ecmwfr", quietly = TRUE)) {
    stop("Package 'ecmwfr' is required for download_era5_cds(). ",
         "Install it with install.packages('ecmwfr').", call. = FALSE)
  }
  ...
  if (length(request_list) <= 20) {
    ...
    stop("The number of requests is too large. Please reduce the number of ",
         "requests to 20 or less.")
  }
