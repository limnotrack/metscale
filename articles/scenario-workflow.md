...
### Calendars: real dates from a model calendar

Climate models don’t all keep the civil (Gregorian) calendar. NIWA’s
CCAM downscaling carries whatever calendar its driving CMIP6 model uses,
and that varies by model: `NorESM2-MM` — bundled here — runs `365_day`
(no leap days, ever); NZESM, NIWA’s other commonly-used downscaled
model, runs `360_day` (twelve uniform 30-day months).
[`extract_climate_point()`](http://limnotrack.com/metscale/reference/extract_climate_point.md)
reads each file’s own CF `calendar` attribute (`calendar = "auto"`, the
default) rather than assuming Gregorian, and decodes accordingly:

- **`standard` / `gregorian` / `proleptic_gregorian` / `julian`** — the
  real civil calendar; decoded directly.
- **`365_day` / `noleap`, `360_day`, `366_day` / `all_leap`** —
  fixed-length calendars with no notion of leap years. Model days are
  mapped onto real `Date`s, and any day the civil calendar doesn’t have
  (29 February in a `365_day` series, 30 February or a 31st month-end in
  `360_day`) is clamped onto the last real day of that month, so the
  resulting series stays gap-free and month-aligned instead of landing
  on a `Date` that doesn’t exist.

Because `NorESM2-MM` is `365_day`, the bundled historical series simply
skips every 29 February:

``` r

any(format(cmip_by$historical$Date, "%m-%d") == "02-29")
#> [1] FALSE
```

A `360_day` calendar clamps *harder*: a non-leap real February has only
28 days but the model gives it 30, so three separate model-days (28th,
29th, 30th) all clamp onto the same real `2000-02-28`. Merging variables
that each carry that duplicated date naively (`merge(..., by = "Date")`)
would not just leave a gap, it would cartesian-multiply rows for that
date across every merged variable.
[`extract_climate_point()`](http://limnotrack.com/metscale/reference/extract_climate_point.md)
collapses each variable’s duplicate dates (by mean) before merging
across variables, specifically to avoid that.

If two variables in the same experiment still don’t share full date
coverage after merging — different calendars, different file lengths —
the mismatch is not left as a silent `NA`: with `verbose = TRUE` a
per-variable NA count is reported, and a
[`warning()`](https://rdrr.io/r/base/warning.html) is raised either way.
Pass `strict = TRUE` to turn that into a hard
[`stop()`](https://rdrr.io/r/base/stop.html) instead, for pipelines that
would rather fail loudly at extraction time than discover a gap several
transform steps later. A missing or empty `calendar` attribute on a file
(`calendar = "auto"`, unable to determine the actual calendar) also
warns before falling back to `standard`, so that silent assumption is at
least visible. See
[`?extract_climate_point`](http://limnotrack.com/metscale/reference/extract_climate_point.md)
for details.

That merge-safe averaging is the right call for the *daily* series
returned by
[`extract_climate_point()`](http://limnotrack.com/metscale/reference/extract_climate_point.md),
but the delta-change step below only ever needs a **monthly mean**, and
computing it from the merged daily series would inherit a small bias:
the clamped days already got averaged into one row, so they’d count as
`1/28`th of February instead of their true `3/30`ths. Use
[`climate_point_monthly_climatology()`](http://limnotrack.com/metscale/reference/climate_point_monthly_climatology.md)
instead — it reads each variable’s own per-file series independently and
averages by calendar month *before* any Date-based merge or
clamp-collapse, so every model day counts equally regardless of a
fixed-length calendar’s month-end clamping. `years` restricts it to a
reference window directly, so there’s no need to filter by year
afterwards.
...
hist_clim <- climate_point_monthly_climatology(
  file.path(ex, "rotorua_cmip6"), lon = lon, lat = lat, vars = names(kind),
  experiments = "historical", years = ref_years, verbose = FALSE)
fut_clim <- climate_point_monthly_climatology(
  file.path(ex, "rotorua_cmip6"), lon = lon, lat = lat, vars = names(kind),
  experiments = c("ssp245", "ssp585"), years = fut_years, verbose = FALSE)

monthly_delta <- function(fut) {
  lapply(names(kind), function(v) {
    a <- hist_clim$value[hist_clim$variable == v][order(hist_clim$month[hist_clim$variable == v])]
    b <- fut$value[fut$variable == v][order(fut$month[fut$variable == v])]
    d <- if (kind[[v]] == "ratio") b / a else b - a
    stats::setNames(d, 1:12)
...
deltas <- list(ssp245 = monthly_delta(fut_clim[fut_clim$experiment == "ssp245", ]),
               ssp585 = monthly_delta(fut_clim[fut_clim$experiment == "ssp585", ]))
...
`ssp585` warms more than `ssp245` in every month. These twelve numbers
are what `apply_delta()` adds to the corresponding months of the
...
the analogue donor day; shortwave is not borrowed but reconstructed from
clear-sky solar geometry, so it starts and ends each day at zero.

Zooming out to a full week shows what disaggregation does to each field
...
total (labelled, mm/day) is left unchanged.
