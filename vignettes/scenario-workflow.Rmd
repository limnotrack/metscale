...
`climprep` covers steps 1 and 3 directly; step 2 is a few lines of arithmetic
...
One winter week, buoy against ERA5 before and after correction. The scale
correction lifts the persistently cold ERA5 air temperature onto the buoy,
scales up the under-forecast wind, and pulls down the humid bias.
...
![plot of chunk correction-week](scenario-workflow-correction-week-1.png)
...
The projected **daily** series (grey step) and the **hourly** series it is
disaggregated into (colour), one week of the `ssp585` 2090--2099 projection.
Air temperature and wind speed vary about their daily mean; rainfall is
redistributed within the day but each day's total is unchanged (printed).
...
![plot of chunk daily-hourly-fig](scenario-workflow-daily-hourly-fig-1.png)
